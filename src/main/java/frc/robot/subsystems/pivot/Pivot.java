...
public class Pivot extends SubsystemBase implements Tuneable {
    ...
    public boolean isAtAngle(double desiredAngle) {
        ...
        boolean isAtAngle = Math.abs(deviation) < IsAtAngle.ANGLE_TOLERANCE;
        SmartDashboard.putBoolean("pivot at angle", isAtAngle);
        fieldsTable.recordOutput("is At angle", isAtAngle);
        return isAtAngle;
    }

    public void stop() {
        fieldsTable.recordOutput("Demand Voltage", 0.0);
        fieldsTable.recordOutput("Real voltage", 0.0);
